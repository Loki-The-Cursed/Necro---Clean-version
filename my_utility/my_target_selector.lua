...
-- Simplified weighted targeting system
local last_scan_time = 0
local cached_weighted_target = nil

local function get_weighted_target(source, min_targets, scan_radius, refresh_rate, comparison_radius)
    local current_time = get_time_since_inject()
    refresh_rate = refresh_rate or 0.2
    scan_radius = scan_radius or 12.0
    comparison_radius = comparison_radius or 3.0
    ...
    if current_time - last_scan_time >= refresh_rate then
        ...
        -- Check if we have minimum targets
        local target_list = target_selector.get_near_target_list(source, scan_radius)
        if #target_list < min_targets then
        ...
        for _, unit in ipairs(target_list) do
            local weight = 1 -- Base weight
            
            -- Add weight based on enemy type
            ...
            -- Add weight for nearby enemies
            local nearby_count = 0
            local unit_pos = unit:get_position()
            for _, other in ipairs(target_list) do
                if other ~= unit then
                    local dist = unit_pos:dist_to(other:get_position())
                    if dist <= comparison_radius then
    nearby_count = nearby_count + 1
end
                end
            end
            weight = weight + (nearby_count * 2)
        ...
        cached_weighted_target = best_target
    end
